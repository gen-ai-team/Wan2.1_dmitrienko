...
def _parse_args():
    ...
    parser.add_argument(
        "--lora_name",
        type=str,
        default="WANARCSH0T111",
        help="The lora to run.",
    )
    parser.add_argument(
        "--size",
        type=str,
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        logging.info(f"Adding LoRa {args.lora_name}...")
        wan_t2v.model = LoraModel(wan_t2v.model, lora_name_to_path)
        wan_t2v.model.apply_lora(args.lora_name, lora_scale=1.0)
        args.prompt = add_lora_tag(args.prompt, args.lora_name)
        ...
        logging.info(f"Adding LoRa {args.lora_name}...")
        wan_i2v.model = LoraModel(wan_i2v.model, lora_name_to_path)
        wan_i2v.model.apply_lora(args.lora_name, lora_scale=1.0)
        args.prompt = add_lora_tag(args.prompt, args.lora_name)
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            args.save_file = (
                f"examples/outputs/{args.task}_{args.size}_{args.ulysses_size}_{args.ring_size}_{formatted_prompt}_{formatted_time}"
                + suffix
            )
