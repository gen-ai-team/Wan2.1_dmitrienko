...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        logging.info("Creating WanT2V_lora pipeline.")
        pipe = wan.WanT2V_lora(
            config=cfg,
    ...
    for lora_dir in args.lora_dirs:
        for lora_scale in args.lora_scales:
            logging.info(f"Applying LoRA from {lora_dir} with scale {lora_scale}...")
            lora_path = os.path.join(lora_dir, "adapter_model.safetensors")
            lora_name_str = ""
            if os.path.exists(lora_path):
                pipe.model.apply_lora(lora_path, lora_scale=lora_scale)
                lora_name_str = f"_{float(lora_scale):.1f}lora"
            if "i2v" not in args.task:
                args.image = [None] * len(args.prompt)
            for prompt, image in zip(args.prompt, args.image):

                logging.info(f"Generating for prompt: {args.trigger_word + prompt}")
                if "t2v" in args.task or "t2i" in args.task:
                    video = pipe.generate(
                        args.trigger_word + prompt,
                    ...
                    if args.crop_image:
                        print(im.size)
                        new_width = int(args.size.split("*")[1])
                        new_height = int(args.size.split("*")[0])

                        if im.size[1] > im.size[0]:
                            im = im.resize(
                                (
                                    new_height,
                                    int(im.size[1] / im.size[0] * new_height),
                                )
                            )
                        else:
                            im = im.resize(
                                (
                                    int(im.size[0] / im.size[1] * new_width),
                                    new_width,
                                )
                            )
                    ...
                    if video is not None:
                        video = video.squeeze(1)[None]
                if rank == 0:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    safe_prompt = prompt.replace(" ", "_").replace("/", "_")[:150]
                    ...
                    save_path = os.path.join(
                        lora_dir,
                        f"{model_name}-{args.task.split('-')[0]}{lora_name_str}_{args.size}_{args.frame_num}f_{args.sample_steps}steps_{args.sample_guide_scale}gs_{args.sample_shift}sh_{args.base_seed}seed/",
                    )
